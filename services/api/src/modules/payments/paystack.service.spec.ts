...
describe('PaystackService', () => {
  ...
  describe('verifyWebhookSignature', () => {
    ...
    it('returns true when signature matches payload hash', () => {
    ...
    it('returns false when signature does not match', () => {
      ...
      expect(service.verifyWebhookSignature(payload, 'wrong-signature')).toBe(false);
    });
    ...
    it('returns false when payload is tampered', () => {
      ...
      expect(service.verifyWebhookSignature(payload + 'x', validSig)).toBe(false);
    });
  });
});
