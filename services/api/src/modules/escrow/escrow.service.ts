...
export class EscrowService {
  ...
  async createEscrow(data: {
    ...
    if (data.useWallet) {
      ...
      const sellerWallet = await this.walletService.getOrCreateWallet(data.sellerId, currency);
    ...
    await this.emailService.sendEscrowCreatedEmail({
  ...
  async shipEscrow(escrowId: string, userId: string, trackingNumber?: string, carrier?: string) {
    ...
    await this.emailService.sendEscrowShippedEmail({
      to: [buyer!.email, seller!.email],
      escrowId: escrow.id,
    });
  ...
  async deliverEscrow(escrowId: string, userId: string) {
    ...
    await this.emailService.sendEscrowDeliveredEmail({
      to: [buyer!.email, seller!.email],
      escrowId: escrow.id,
    });
