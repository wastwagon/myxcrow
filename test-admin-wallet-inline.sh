...
# Test Admin Wallet Management (runs from inside container)
...
docker exec escrow_api bash <<EOF
set -e
...
psql -U postgres -d escrow -c "UPDATE \\\"User\\\" SET roles = ARRAY['ADMIN']::\\\"UserRole\\\"[] WHERE id = '\$ADMIN_ID';" > /dev/null 2>&1
...
echo "Credit Response: \$CREDIT_RESPONSE"
...
echo "Debit Response: \$DEBIT_RESPONSE"
