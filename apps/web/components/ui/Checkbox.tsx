...
export const Checkbox = forwardRef<HTMLInputElement, CheckboxProps>(
  ({ className, tone = 'light', label, id, ...props }, ref) => {
    const input = (
      <input
        ref={ref}
        id={id}
        type="checkbox"
        className={cn(
          'h-5 w-5 rounded border focus:ring-2 focus:ring-offset-0 touch-manipulation',
          tone === 'dark'
    ...
    return (
      <label
        htmlFor={id}
        className={cn(
          'inline-flex min-h-[44px] items-center gap-2.5 cursor-pointer select-none touch-manipulation',
