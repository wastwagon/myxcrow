...
export default function ProfilePage() {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: { firstName?: string; lastName?: string }) => {
      // Try to update via users endpoint first, fallback to direct API call
      try {
        const response = await apiClient.put(`/users/${user?.id}`, data);
        return response.data;
      } catch (error: any) {
        // If users endpoint doesn't exist, try auth/profile endpoint
          ...
          {isLoading ? (
            ...
            <>
              <div className="flex items-center gap-4 pb-6 border-b">
                ...
                <div>
                  <h2 className="text-2xl font-semibold text-gray-900">{displayUser?.email}</h2>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-900">
                        {displayUser?.createdAt ? formatDate(displayUser.createdAt) : 'N/A'}
                      </p>
                    </div>
                  </div>
